...
function encodeTransaction (inputs_from_api, fee, senders_public_key, senders_address, send_amount, receipient_address, callback) {
  // init empty tx
  ...
  transaction.siacoinOutputs.push({
    value: BigInt(send_amount * 1000000000000000000000000).toString(),
    unlockHash: receipient_address
  })
  // add change output
  transaction.siacoinOutputs.push({
    value: BigInt(totalInputAmount - BigInt(send_amount * 1000000000000000000000000)).toString(),
    unlockHash: senders_address
  })
  // add miner fees
  transaction.minerFees.push(BigInt(1000000000000000000000000 * fee).toString())
  // add signatures
  for (var i = 0; i < inputs_from_api.length; i++) {
    transaction.transactionSignatures.push({
      ...
      signature: sigHash(transaction, inputs_from_api[i].output, 0, 0)
    })
  }
