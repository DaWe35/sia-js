...
function generateRandomData (callback = null) {
  if (!hasStrongRandom()) {
    var errorText = "This browser does not support strong randomness, please use a modern one"
  ...
  // create secure entropy asyncronously
  if (callback != null) {
    // async randomBytes if callback is not null
    crypto.randomBytes(256, (err, randomData) => {
      if (err) throw err;
      callback(randomData)
      return
    });
  } else {
    // Synchronous random generation
    let randomData = crypto.randomBytes(32)
    return randomData
  }
}
...
// derives an address from the given private key and return an object with address, privateKey, and publicKey
...
// generates a public key fron a given private key
